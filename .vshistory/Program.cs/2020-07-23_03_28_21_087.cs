...
namespace ArmA3_SaveCleaner
{
    internal class Program
    {
        ...
        private static void Main()
        {
            ...
            // toDelete.Reverse();
            foreach (var file in toDelete) Console.WriteLine($"{file.FullName.Quote()} ({file.Length.Bytes().Humanize("MB")})");
            Console.WriteLine();
            Utils.Console.Confirm($"Found {toDelete.Count} files in {searchedPaths} directories. Delete them now?");
