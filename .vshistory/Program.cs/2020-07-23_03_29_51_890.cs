...
namespace ArmA3_SaveCleaner
{
    internal class Program
    {
        ...
        private static void Main()
        {
            ...
            // toDelete.Reverse();
            long totalSize = 0;
            foreach (var file in toDelete) { Console.WriteLine($"{file.FullName.Quote()} ({file.Length.Bytes().Humanize("MB")})"); totalSize += file.Length; }
            Console.WriteLine();
            Utils.Console.Confirm($"Found {toDelete.Count} files taking up {} in {searchedPaths} directories. Delete them now?");
